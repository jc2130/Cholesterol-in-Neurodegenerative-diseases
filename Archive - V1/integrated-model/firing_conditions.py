...
fc_t_SNCA_bind_ApoEchol_extra = lambda a : a['p_ApoEchol_extra'] > 0 and a['p_SNCA_act'] > 1.001 * it_p_SNCA_act 

fc_t_chol_LE_upreg= lambda a : a['p_GBA1']>0 # PD specific
fc_t_TRADD_actv = lambda a : a['p_chol_LE'] >=0


# Lewy body pathology
fc_t_SNCA_degr = lambda a : a['p_SNCA_act']>0
fc_t_SNCA_aggr = lambda a : a['p_SNCA_act']>5.21e8
...
fc_t_RTN3_aggregation = lambda a : (a["p_RTN3_axon"] + a["p_RTN3_PN"]) > (it_p_RTN3_tot * (1 - (1-dec_t_RTN3_aggregation) * (a['p_Ab'] - it_p_Ab)/(Ab_t_RTN3_aggregation - it_p_Ab))) and a['p_RTN3_axon'] > 100 and a['p_RTN3_PN'] > 100 # proportional reduction in RTN3 aggregation threshold based on Ab level
...
fc_t_ETC = lambda a : a['p_reduc_mito'] > 1 and a['p_ADP'] > 1
fc_t_ROS_metab = lambda a : a['p_ROS_mito'] > 1
fc_t_mito_dysfunc = lambda a : a['p_ROS_mito'] > 1
